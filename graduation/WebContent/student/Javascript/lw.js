	...
	$("#lwB").click(
			function() {
				if ($("#wj").val() == null || $("#wj").val() == "") {
					...
					$("#lwB").css("display", "none").parent()
							.find(".addGBajax").css("display", "inline-block");
					lwUpload();
				}
			});
...
function lwUpload() {
	var formData = new FormData($("#lwwj")[0]);
	$.ajax({
		url : "upload.do?meth=lw",
		type : 'POST',
		data : formData,
		async : false,
		cache : false,
		...
		success : function(data) {
			showMsg(data.msg, "center", "window.location.href = \"zzlw.do\"");
			$("#lwB").css("display", "inline-block").parent()
					.find(".addGBajax").css("display", "none");
		},
		error : function(data) {
			alert("connect error!");
			window.location.href = "zzlw.do";
		}
	});
}
						...
						if (data.status == 200) {
							if (data.k == 1) {// 代表新的
								...
								$ne = '<div class="line_grid"><form class="jdxcgswj" enctype="multipart/form-data">'
										...
										+ '<div class="simple_buttons">'
										+ '<div onclick="window.open(\'preview.do?meth=lwsLoad&id='
										+ data.id
