...
<%
	...
	List<Lws> lws = ServiceFactory.getTeacherServiceInstance().getStudentLws(xuehao);
%>
<!DOCTYPE html>
<html>
<head>
...
<title><%=student.getName()%>同学的最终成果详情</title>
		...
		<div class="contents" style="margin: 0 auto; float: none;">
			...
			<div class="grid_wrapper">
				...
				<div class="g_6 contents_header">
					<h3 class="i_16_dashboard tab_label"><%=student.getName()%>同学的最终成果详情
					</h3>
					<div>
						<span class="label">阅览学生的最终成果，依此评分</span>
					</div>
				</div>
				...
				<div class="g_12">
					<div class="widget_contents noPadding">
						<div class="line_grid">
							...
							<div class="g_3">
								<a target="_blank"
									href="lw.do?meth=preview&number=<%=request.getParameter("number")%>"><input
									type="button" value="阅览" class="submitIt simple_buttons" /></a>
									<a target="_blank"
									href="lw.do?meth=load&number=<%=request.getParameter("number")%>"><input
									type="button" value="下载" class="submitIt simple_buttons" /></a>
							</div>
						</div>
						<%
							for (int x = 0, y = (lws == null ? 0 : lws.size()); x < y; x++) {
						%>
						<div class="line_grid">
							<div class="g_9">
								<span class="label"><%=lws.get(x).getFileName() %>：
								</span>
							</div>
							<div class="g_3">
								<a target="_blank"
									href="preview.do?meth=lws&id=<%=lws.get(x).getId()%>"><input
									type="button" value="阅览" class="submitIt simple_buttons" /></a>
									<a target="_blank"
									href="preview.do?meth=lwsLoad&id=<%=lws.get(x).getId()%>"><input
									type="button" value="下载" class="submitIt simple_buttons" /></a>
							</div>
						</div>
						<%
